...
def index():
    ...
    if 'status' in req and int(req['status']) == -10:
        query = query.filter(PayOrder.status > int(- 10))
...
def account():
    ...
    stat_info = db.session.query(func.sum(PayOrder.pay_price)).filter(PayOrder.status == 1).first()
